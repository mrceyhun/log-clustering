...
class OptionParser():
    def __init__(self):
        ...
        self.parser.add_argument("--creds", action="store",
                                 dest="creds", default="",
                                 help="Stomp AMQ credentials file, if provided the data will be send to MONIT")
        ...
        self.parser.add_argument("--filter", action="store",
                                 dest="filter", default="",
                                 help="filter condition to apply to FTS logs, e.g. vo == 'cms'")
...
def run(creds, fout, date=None, ifilter=None):
    ...
    fts_df = fts_tables(sc, date=date, schema=_schema).select(
        ...
        'error_message <> ""')  # taking non-empty messages, if date is not given then the records from yesterday are taken
    if ifilter:
        fts_df = fts_df.where(ifilter)

    # Print only 1 row of pyspark df
    fts_df.show(1)

    df = fts_df.toPandas()  # the messages are converted to Pandas df

    mod_name = 'word2vec_' + time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime(
        time.time())) + '.model'  # model is named according to the run date
    ...
    for el in a:
        ...
        df.loc[cluster.result.loc[el, 'indices'], 'cluster_pattern'] = cluster.result.loc[
            el, 'pattern']  # info about the clusters is added to the error messages
    ...
    if creds:
        ...
        stomp_amq = StompAMQ(username=username, password=password, producer=producer, topic=topic, key=ckey, cert=cert,
                             validation_schema=None, host_and_ports=[(host, port)], loglevel=logging.WARNING)
        for d in df_to_batches(res, 10000):
            messages = []
            for msg in d:
